...
TIMES="10"
CHATID="6198984094"
KEY="6919563413:AAHXyYj4CUED_DolztFh6oqXQ6FzyzwICK8"
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/king-vpn/first/master/REGIST | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/king-vpn/first/master/REGIST | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/king-vpn/first/master/REGIST | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
echo -e "   '----------------------------------'"
echo -e "     \e[1;32m1)\e[0m Domain Sendiri ( recommended )"
...
#install cf
wget ${REPO}file/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
...
bash -c "$(curl -L https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install -u www-data --version $latest_version
    ...
    # // Ambil Config Server
    wget -O /etc/xray/config.json "${REPO}file/config.json" >/dev/null 2>&1
...
print_install "Memasang modul SlowDNS Server"
    wget -q -O /tmp/nameserver "${REPO}file/nameserver" >/dev/null 2>&1
